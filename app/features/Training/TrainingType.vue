<script lang="ts" setup>
  ...
  import type { IClimbingTraining, ILiftingTraining, IRunningTrainig, ITraining } from '~/models/ITraining'
  ...
  const training = computed((): ITraining | undefined => {
    switch (type.value) {
      case 'running':
        return {
          ...
          risk_watch: ['sztywność soleusa', 'RPE >6'],
          abort_conditions: ['ból łydki', 'RPE >7 utrzymujące się >5 minut'],
          notes: ['Jeśli RPE rośnie, skrócić część główną o 5–10 minut'],
        } as IRunningTrainig;
      case 'lifting':
        return {
          ...
          stop_conditions: ['RPE >=9', 'RIR <=0 w dwóch seriach z rzędu'],
          notes: [
      ...
      case 'climbing':
        return {
          ...
          quality_checks: ['utrata precyzji stóp', 'spadek kontroli ciała'],
          stop_conditions: ['spadek jakości ruchu w 2 kolejnych drogach', 'RPE >8'],
          notes: [
            'Nie dokładać trudności kosztem jakości',
            'Zakończyć sesję przy pierwszych oznakach chaosu ruchowego',
          ],
        } as IClimbingTraining;
      default:
        return undefined;
    }
  })
...
<template>
  <div class="p-8">
    <component
        v-if="training !== undefined"
        :is="trainingComponent"
        :selected-date="props.selectedDate || new Date()"
